...
form.addEventListener('submit',function(e){
  ...
  if(isNaN(value) || value ===1 || value<0 || value>9){
    result.innerHTML = `<p class="result">${text[Math.floor(Math.random()*9)]}</p>`
  } else{
    // 1) myfirst
    // var para =``;
    // for (var i=0; i<value;i++){
    //   para = para+`<p class="result">${text[i]}</p>`;
    // }

    // 2)another
    let para = text.slice(0,value);
